...
// kcal per gram: protein=4, carbs=4, fat=9, fiber=2
...
function MacroInput({ label, field, value, color, onChange }: MacroInputProps) {
  ...
  return (
    <div className="bg-[#1a1a1a] rounded-xl px-4 py-3">
      ...
      <div className="flex items-center gap-3">
        <input
          type="range" min={0} max={MACRO_MAX[field]} value={value}
...
export default function ProfilePage({ macroTargets, weightEntries, onSaveMacroTargets, onAddWeightEntry, onDeleteWeightEntry }: Props) {
    ...
    <div className="flex-1 overflow-y-auto px-4 pt-4 pb-24 space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="bg-[#1a1a1a] rounded-xl px-4 py-3 text-xs text-[#737373] space-y-0.5">
          <p>As cores no resumo de hoje mudam consoante o consumo:</p>
          <div className="flex gap-4 mt-1.5">
            ...
            <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-yellow-400 inline-block" />+10% acima</span>
            <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-red-500 inline-block" />+20% acima</span>
          </div>
        </div>
